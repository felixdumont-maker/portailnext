...
export interface Logo        { public_url: string | null; preview_url: string | null; filename: string | null }
export interface PaletteColor{ hex: string; label?: string }
export interface Font        { nom_font: string; google_font_url: string | null; usage: string | null }
export interface ImageAsset  { id: number; public_url: string | null; label: string | null; filename: string | null }
...
function LoadingSkeleton() {
  return (
    <div>
      ...
      <div style={{ marginTop: 'var(--space-8)', display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 'var(--space-4)' }}>
...
// ─── Main content (fetch + render) ──────────────────────────

export function IdentiteVisuelleContent({
  projetId, showHeader = false,
}: { projetId: string | string[] | undefined; showHeader?: boolean }) {
  const [data,    setData]    = useState<IdentiteData | null>(null)
  const [loading, setLoading] = useState(true)
  const [error,   setError]   = useState(false)

  useEffect(() => {
    fetch(`${API}/api/v1/projet/${projetId}/identite`, { credentials: 'include' })
      .then(r => {
  ...
  if (error || !data) return (
    <p style={{ fontFamily: 'var(--font-body)', color: 'var(--color-light-text-3)', fontSize: 'var(--text-sm)' }}>
      ...
      {hasPalette && (
        <section style={{ marginBottom: 'var(--space-16)' }}>
          ...
          <div style={{
            ...
            height: '80px', marginBottom: 'var(--space-6)',
          }}>
      ...
      {hasLogos && (
        <section style={{ marginBottom: 'var(--space-16)' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' as const, gap: 'var(--space-10)' }}>
            <LogoGroup title="Version noire" keys={NOIR_KEYS} dark={false} bg="var(--color-light-0b)" logos={data.logos} extras={noirExtras} />
            <LogoGroup title="Version blanche" keys={BLANC_KEYS} dark={true} bg="var(--color-dark-1)" logos={data.logos} extras={blancExtras} />
              ...
              <div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 'var(--space-5)' }}>
                  {autresLogos.map(({ key, logo }) => (
                    <LogoCard key={key} varKey={key} label={humanLabel(key)} bg="var(--color-light-0b)" dark={false} logo={logo} />
                  ))}
                </div>
              </div>
            )}
              ...
              <div key={i} style={{
                ...
                {font.usage && (
                  <span style={{
                    fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', fontWeight: 700,
                    textTransform: 'uppercase' as const, letterSpacing: '0.12em',
                    color: 'var(--color-light-text-3)', display: 'block', marginBottom: 'var(--space-4)',
                  }}>
                    {font.usage}
                  </span>
                )}
                <p style={{
                  fontFamily: `'${font.nom_font}', serif`, fontSize: 'clamp(2rem, 6vw, 3.5rem)',
                  lineHeight: 1.05, color: 'var(--color-light-text)', margin: '0 0 var(--space-4)', letterSpacing: '-0.02em',
                }}>
                  {font.nom_font}
                </p>
                <p style={{
                  fontFamily: `'${font.nom_font}', serif`, fontSize: 'var(--text-lg)',
                  lineHeight: 'var(--leading-relaxed)', color: 'var(--color-light-text-2)', margin: '0 0 var(--space-4)', maxWidth: '50ch',
                }}>
                  À chaque projet, une identité unique.
                </p>
                <p style={{
                  fontFamily: `'${font.nom_font}', serif`, fontSize: 'var(--text-sm)',
                  color: 'var(--color-light-text-3)', margin: '0 0 var(--space-4)',
                  letterSpacing: '0.05em', lineHeight: 'var(--leading-relaxed)',
                }}>
                ...
                {font.google_font_url && (
                  <a href={font.google_font_url} target="_blank" rel="noreferrer" style={{
                    fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', fontWeight: 700,
        ...
        <section style={{ marginBottom: 'var(--space-16)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 'var(--space-5)' }}>
            {data.declinaisons.map(d => (
              <div key={d.id}>
                <div style={{
                  borderRadius: 'var(--radius-md)', overflow: 'hidden', background: 'var(--color-light-0)',
                  border: '1px solid var(--color-light-border)', aspectRatio: '4/3',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
                  {d.public_url ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={d.public_url} alt={d.label || ''} style={{ width: '100%', height: '100%', objectFit: 'contain', padding: 'var(--space-6)' }} />
                  ) : (
              ...
              <div key={m.id} style={{
                ...
                {m.label && (
                  <div style={{ padding: 'var(--space-3) var(--space-4)' }}>
                    <span style={{
                      fontFamily: 'var(--font-body)', fontSize: 'var(--text-xs)', fontWeight: 700,
                      textTransform: 'uppercase' as const, letterSpacing: '0.10em', color: 'var(--color-light-text-3)',
                    }}>
                      {m.label}
                    </span>
                  </div>
