...
function SectionBlock({ section, index, open, onToggle, captures, onSelectCapture }: {
  ...
  return (
    <div id={`section-${section.id}`} style={{
      ...
      >
        <span style={{
          fontFamily: FONT_TITLE, fontSize: '20px', letterSpacing: '1px', color: BRAND, flexShrink: 0,
        }}>
          {numero(index)}
        </span>
        <span style={{
...
export default function GuideModifierSiteShopifyPage() {
      ...
      <div style={{ maxWidth: '760px', margin: '0 auto', padding: '64px 24px 100px' }}>
        ...
        <div style={{ marginBottom: '40px' }}>
          ...
          <h1 style={{
            ...
            Comment modifier votre site Shopify
          </h1>
          <p style={{ fontFamily: FONT_BODY, fontSize: '15px', lineHeight: 1.8, color: '#5a554f', margin: 0, maxWidth: '600px' }}>
            Ce guide vous montre, étape par étape, comment effectuer les modifications les plus courantes
            sur votre boutique Shopify : textes, images, produits, collections et menu de navigation.
        ...
        {/* Table des matières */}
        <div style={{
          display: 'flex', flexWrap: 'wrap' as const, gap: '8px', marginBottom: '32px',
        }}>
          {sections.map((s, i) => (
            <button
              key={s.id}
